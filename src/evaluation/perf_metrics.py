...
from evaluation.structures import PerformanceResults
...
class AllPlayerMetrics(PerformanceResults):
    ...
    def reduce_to_score(self) -> float:
        """Higher is better"""
        return sum(self.get_score_details().values())

    def get_score_details(self) -> dict:
        score_components = {}
        score_components["goals_delivered"] = self.num_goals_delivered * 100
        score_components["collisions"] = -self.num_collided_players * 500
        score_components["time_bonus"] = (self.max_sim_time - self.task_accomplishment_time) * 10
        score_components["distance_penalty"] = -self.total_travelled_distance * 0.5
        score_components["actuation_penalty"] = -self.total_actuation_effort * 0.01
        score_components["comp_time_penalty"] = -max(0.0, self.avg_computation_time - 0.1) * 100
        score_components["global_plan_penalty"] = -max(0.0, self.global_planning_time - 60) * 10
